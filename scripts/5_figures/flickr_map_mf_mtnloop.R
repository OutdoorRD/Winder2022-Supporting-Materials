...
##############################
## Ok. Similar map but only for Skyomish RD
#flickr_s
#
#rd = "Skykomish"
#
#skykomish <- ggplot() +
#  geom_sf(data = MBS %>% filter(Name_short == "Skykomish"), aes(fill = "nonwild")) +
#  #scale_fill_brewer(palette = "Greys") +
#  geom_sf(data = instag_mbs_sf %>% filter(Name_short == "Skykomish"), aes(size = IUD, col = 'c'), alpha = .4) +
#  geom_sf(data = alltrails_mbs_sf %>% filter(Name_short == "Skykomish"), aes(size = AUD, col = 'd'), alpha = .4) +
#  geom_sf(data = flickr_mbs_sf %>% filter(Name_short == "Skykomish"), aes(col = 'a'), alpha = .3, size = 1) +
#  geom_sf(data = twitter_mbs_sf %>% filter(Name_short == "Skykomish"), aes(col = 'b'), alpha = .2, size = 1) +
#  geom_sf(data = wta_mbs_sf %>% filter(Name_short == "Skykomish"), aes(size = WUD, col = 'e'), alpha = .4) +
#  geom_sf(data = mbs_wilds %>% filter(Name_short == "Skykomish"), aes(fill = 'wild'), alpha = .6) +
#  scale_colour_manual(name = 'Social Media', 
#                      values =c('a'='#7b3294','b'='#1a9641', 'c' = '#0571b0', 'd' = "red", 'e' = "yellow"), 
#                      labels = c("Flickr", "Twitter", "Instagram", "AllTrails", "WTA")) + #, guide = "none") +
#  scale_size_continuous(range = c(.5, 15), name = "Instagram/AllTrails/WTA User-days") + #, guide = "none") +
#  scale_fill_manual(name = NULL, 
#                    values = c('nonwild' = 'gray80', 'wild' = 'white'), 
#                    labels = c("Non-Wilderness", "Wilderness")) +
#  #geom_sf_label(data = MBS, aes(label = Name_short), nudge_x = 0) +
#  #coord_sf(xlim = c(-122.1, -120.7), ylim = c(46.68, 49)) +
#  annotation_scale(location = "bl", style = "ticks") +
#  #coord_sf(datum = NA) +
#  xlab(NULL) +
#  ylab(NULL) +
#  labs(title = "Social Media Posts in Skykomish Ranger District") +
#  guides(fill = guide_legend(order = 1), col = guide_legend(order = 2), size = guide_legend(order = 3)) +
#  theme_void() +
#  theme(axis.text = element_blank(), axis.ticks = element_blank(), panel.grid.major = element_line(color = "transparent"))
#
#skykomish
#
## let's write it out
#ggsave(paste0("figs/skykomish_map_", dddd, ".png"), plot = skykomish, width = 8, height = 7, units = "in")
#
### HOw about without the wilderness
#
#skykomish_nw <- ggplot() +
#  geom_sf(data = MBS %>% filter(Name_short == "Skykomish"), aes(fill = "nonwild")) +
#  #scale_fill_brewer(palette = "Greys") +
...
# # geom_sf(data = mbs_wilds %>% filter(Name_short == "Skykomish"), aes(fill = 'wild'), alpha = .6) +
#  scale_colour_manual(name = 'Social Media', 
#                      values =c('a'='#7b3294','b'='#1a9641', 'c' = '#0571b0', 'd' = "red", 
#                                'e' = "#FFD700"), 
#                      labels = c("Flickr", "Twitter", "Instagram", "AllTrails", "WTA")) + #, guide = "none") +
#  scale_size_continuous(range = c(.5, 15), name = "Instagram/AllTrails/WTA \nUser-days",
#                        trans = "log10") + #, guide = "none") +
#  scale_fill_manual(name = NULL, 
#                    values = c('nonwild' = 'gray80'),#, 'wild' = 'white'), 
#                    labels = c("Skykomish RD"))+#, "Wilderness")) +
#  #geom_sf_label(data = MBS, aes(label = Name_short), nudge_x = 0) +
#  #coord_sf(xlim = c(-122.1, -120.7), ylim = c(46.68, 49)) +
#  annotation_scale(location = "bl", style = "ticks") +
#  #coord_sf(datum = NA) +
#  xlab(NULL) +
#  ylab(NULL) +
#  labs(caption = "Outdoor Recreation & Data Lab, November 2020") +
#  guides(fill = guide_legend(order = 4), col = guide_legend(order = 2), size = guide_legend(order = 3)) +
#  theme_void() +
#  theme(axis.text = element_blank(), axis.ticks = element_blank(), 
#        panel.grid.major = element_line(color = "transparent"),
...
#skykomish_nw
## let's write it out
#ggsave(paste0("figs/skykomish_map_no_wilds_", dddd, ".png"), plot = skykomish_nw, width = 8, height = 7, units = "in")
#
#
#
### Same for Darrington
#
#Darrington <- ggplot() +
#  geom_sf(data = MBS %>% filter(Name_short == "Darrington"), fill = "grey95") +
#  #scale_fill_brewer(palette = "Greys") +
#  geom_sf(data = instag_mbs_sf %>% filter(Name_short == "Darrington"), aes(size = iud, col = 'c'), alpha = .4) +
#  geom_sf(data = flickr_mbs_sf %>% filter(Name_short == "Darrington"), aes(col = 'a'), alpha = .3, size = 1) +
#  geom_sf(data = twitter_mbs_sf %>% filter(Name_short == "Darrington"), aes(col = 'b'), alpha = .2, size = 1) +
#  scale_colour_manual(name = 'Social Media', 
#                      values =c('a'='#7b3294','b'='#1a9641', 'c' = '#0571b0'), 
#                      labels = c("Flickr", "Twitter", "Instagram")) + #, guide = "none") +
#  scale_size_continuous(range = c(1, 15), name = "Instagram User-days") + #, guide = "none") +
#  #geom_sf_label(data = MBS, aes(label = Name_short), nudge_x = 0) +
#  #coord_sf(xlim = c(-122.1, -120.7), ylim = c(46.68, 49)) +
#  annotation_scale(location = "bl", style = "ticks") +
#  #coord_sf(datum = NA) +
#  xlab(NULL) +
#  ylab(NULL) +
#  labs(title = "Social Media Posts in Darrington Ranger District") +
#  theme_void() +
#  theme(axis.text = element_blank(), axis.ticks = element_blank(), panel.grid.major = element_line(color = "transparent"))
#
#Darrington
#
## let's write it out
##ggsave(paste0("figs/Darrington_map_", dddd, ".png"), plot = Darrington, width = 8, height = 7, units = "in")
#
#
#
#
